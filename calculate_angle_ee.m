function [res] = calculate_angle_ee(joint_angles_sym, transformation_matrix_ee_sym, previous_joint_angle_vector, waypoint, joint_angles_1_3_numeric)

transformation_matrix_ee_sym = subs(transformation_matrix_ee_sym, joint_angles_sym(1:3), joint_angles_1_3_numeric);
transformation_matrix_ee_sym = subs(transformation_matrix_ee_sym, joint_angles_sym(6), 0.0);

joint_angles_sym = joint_angles_sym(4:5);
previous_joint_angle_vector = previous_joint_angle_vector(4:5);
waypoint = waypoint(4:end);

num_joints = size(joint_angles_sym, 2);
num_pose_variables = size(waypoint, 2);

assert(isequal(size(joint_angles_sym), [1,num_joints]))
assert(isequal(size(transformation_matrix_ee_sym), [4, 4]))
assert(isequal(size(previous_joint_angle_vector), [1,num_joints]))
assert(isequal(size(waypoint), [1,num_pose_variables]))
assert(isequal(size(joint_angles_1_3_numeric), [1,3]))
...
angle_constraints_in_radians = angle_constraints_in_radians(4:5, :);
assert(isequal(size(angle_constraints_in_radians), [num_joints, 2]))
tip_pos_numeric = calc_pose(joint_angles_sym, current_joint_angle_vector, transformation_matrix_ee_sym);
tip_pos_numeric = tip_pos_numeric(4:end);
...
while dist_to_waypoint_scalar > 1e-12
    ...
    epsilon = 1e-6;
    jacobian_numeric = my_jacobian(joint_angles_sym, current_joint_angle_vector, transformation_matrix_ee_sym, epsilon);
    jacobian_numeric = jacobian_numeric(4:end, :);
